...
public:
    ...
    //Checks if the key exists in the AVL tree
    bool exists(const T& key) {
        return findNode(root, key) != nullptr;
    }

    //Checks for and deletes a key from the AVL tree
    //If the entry does not exist, it will inform the user.
    void remove(T key)
    {
        if (findNode(root, key)) {
            ...
            cout << "Book not found. Delete failed. \n";
        }
    }
